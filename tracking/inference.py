...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # From lecture slide 2/27; bayes rule: P(noisyD | emission) = P(emission | noisyD) * P(noisyD)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        # from lecture slides 2/27, and the code in the comments above
        allPossible = util.Counter()
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        beliefsOld = self.getBeliefDistribution()

        # special case when ghost eaten
        if noisyDistance is None:
            jail = self.getJailPosition()
            self.particles = [jail] * self.numParticles
        else:
            for pos in self.legalPositions:
                dist = util.manhattanDistance(pacmanPosition, pos)
                allPossible[pos] += emissionModel[dist] * beliefsOld[pos]
            if allPossible.totalCount() == 0:  # special case 2
                self.initializeUniformly(gameState)
            else:
                temp = []
                p = 0
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # basically like initialize uniformly particle from before but with itertools
        iterPosition = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(iterPosition)
        sum = 0
        self.particles = []
        while sum < self.numParticles:
            for p in iterPosition:
                if sum < self.numParticles:
                    self.particles.append(p)
                    sum += 1
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] is None:  # special case 1, ghost in jail
                    particle = self.getParticleWithGhostInJail(particle, ghost)
                else:
                    distance = util.manhattanDistance(particle[ghost], pacmanPosition)
                    partial *= emissionModels[ghost][distance]
            allPossible[particle] += partial

        if allPossible.totalCount() == 0:  # special case 2, all weight zero
            self.initializeParticles()
            for particle in self.particles:
                for ghost in range(self.numGhosts):
                    if noisyDistances[ghost] is None:
                        particle = self.getParticleWithGhostInJail(particle, ghost)
        else:  # basically like the particle filter from before
            allPossible.normalize()
            temp = []
            p = 0
            while p < self.numParticles:
